...
def add_card(card: dict):
    ...
    cursor = conn.cursor()
    try:
        cursor.execute("INSERT INTO cards (name, evolution_stage, rarity, type, hp, move, effect, damage, weakness, resistance, retreat_cost) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                       (card['name'], card['evolution_stage'], card['rarity'], card['type'], card['hp'], card['move'], card['effect'], card['damage'], card['weakness'], card['resistance'], card['retreat_cost']))
        conn.commit()
