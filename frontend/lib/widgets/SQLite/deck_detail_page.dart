// frontend\lib\widgets\SQLite\deck_detail_page.dart
...
class DeckDetailPage extends StatefulWidget {
  ...
  const DeckDetailPage({super.key, required this.deckId, required this.cardId});
...
class _DeckDetailPageState extends State<DeckDetailPage> {
  ...
  Future<void> addCardToDeck() async {
    try {
      debugPrint(
          '正在添加卡片到牌組 - Deck ID: ${widget.deckId}, Card ID: ${widget.cardId}');
      await DeckDatabase.instance.addCardToDeck(widget.deckId, widget.cardId);
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('卡片已成功添加到牌組')),
      );
    } catch (e) {
      debugPrint('錯誤信息: $e');
      ScaffoldMessenger.of(context).showSnackBar(
      ...
      body: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 16),
            ElevatedButton(
              onPressed: addCardToDeck,
              child: const Text('新增至牌組'),
            ),
            const SizedBox(height: 16),
